...
use zintl_native::{
    Context, MainActor, Rect, Window, WindowAppMenu as NativeWindowAppMenu, WindowCommandEvent,
...
impl AppWindowState {
    pub(crate) fn handle_message(&self, message: Message) {
        match message {
            Message::WindowOperationCompleted {
                operation_id,
                result,
            } => {
                if let WindowOperationResult::Create(Ok(window_id)) = &result {
                    self.push_app_event(ZintlAppEvent::WindowCreated {
                        window_id: *window_id,
                    });
                }
