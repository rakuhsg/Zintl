...
Dispatch looks up the immutable descriptor and validates request metadata and
input size before yielding `PreparedOp`. The embedding API uses an internal
allow policy because filesystem authorization now belongs to each VFS
`Authority`, not to a runtime-wide callback.
...
executor must enqueue without blocking the JS executor.

The registered async closure is run by an explicit `HostOpExecutor`, not by
MainActor or the JSC serial executor.

The descriptor timeout covers host execution.
